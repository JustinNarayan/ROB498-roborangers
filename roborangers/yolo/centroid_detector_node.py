#!/usr/bin/env python3
"""ROS2 node for IMX219 YOLOv5 detection and centroid publishing."""
...
from roborangers.yolo.imx219_yolov5_onnx import (
    YoloV5OnnxDetector,
    draw_detections,
    gstreamer_pipeline,
    load_class_names,
...
class CentroidDetectorNode(Node):
    def __init__(self) -> None:
        ...
        use_cpu = bool(self.get_parameter("use_cpu").value)

        self.detector = YoloV5OnnxDetector(
        ...
        self.get_logger().info("Centroid detector node started")
    ...
    def _resolve_target_class_id(self) -> Optional[int]:
        ...
        if self.target_class_name not in self.class_names:
            self.get_logger().warn(
                f"target_class_name '{self.target_class_name}' not found in classes; using top detection"
            )
            return None
