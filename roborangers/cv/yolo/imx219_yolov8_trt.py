...
def resolve_target_class_id(
    ...
    if target_class_id is not None and target_class_id >= 0:
        return target_class_id

    normalized_name = (target_class_name or "").strip()
    if class_names:
        if normalized_name and normalized_name in class_names:
            return class_names.index(normalized_name)
...
def filter_detections_by_class(
    detections: Sequence[Detection], target_class_id: Optional[int]
) -> List[Detection]:
    if target_class_id is None:
        return list(detections)
    return [det for det in detections if det.class_id == target_class_id]
...
def camera_backend_diagnostic() -> str:
    cv2_path = getattr(cv2, "__file__", "<unknown>")
    guidance = [
        "OpenCV was built without GStreamer support, so cv2.CAP_GSTREAMER cannot open nvarguscamerasrc.",
        f"Active cv2 module: {cv2_path}",
    ]

    if "/.local/" in cv2_path or "site-packages" in cv2_path:
        guidance.append(
            "This usually means a pip-installed opencv-python wheel is shadowing Jetson's system OpenCV build."
        )

    guidance.extend(
        [
            "On Jetson, use the L4T/JetPack OpenCV build with GStreamer enabled, or rebuild OpenCV locally with GStreamer support.",
            "Quick check: python3 - <<'PY'\nimport cv2\nprint(cv2.__file__)\nprint('GStreamer:', 'YES' if 'GStreamer:                   YES' in cv2.getBuildInformation() else 'NO')\nPY",
        ]
    )
    return "\n".join(guidance)
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Optional class id to visualize. Use -1 to show all detections.",
    )
    parser.add_argument(
        "--target-class-name",
        default="",
        help="Optional class name to visualize. Ignored when --target-class-id is set.",
    )
    parser.add_argument("--input-size", type=int, default=640)
    parser.add_argument("--conf", type=float, default=0.35)
    parser.add_argument("--iou", type=float, default=0.45)
    parser.add_argument("--camera-width", type=int, default=1640)
    parser.add_argument("--camera-height", type=int, default=1232)
    parser.add_argument("--display-width", type=int, default=1640)
    parser.add_argument("--display-height", type=int, default=1232)
...
def main() -> None:
    ...
    if explicit_target_class_id is not None:
        ...
        print("Showing detections from all classes")
    ...
    try:
        while True:
            ...
            filtered_detections = filter_detections_by_class(detections, target_class_id)
            vis = draw_detections(frame, filtered_detections, class_names=class_names)
            ...
            if filtered_detections:
                cx, cy = filtered_detections[0].centroid
                print(
                    f"det={len(filtered_detections)} top_cls={filtered_detections[0].class_id} "
                    f"conf={filtered_detections[0].confidence:.3f} cx={cx:.1f} cy={cy:.1f}"
                )
